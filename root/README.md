...
Proiectul implementează un sistem de navigație urbană care generează rute folosind un graf rutier real, construit cu Python (OSMnx + NetworkX).
Sunt disponibile mai multe tipuri de rutare (distanță, timp, echilibrată), precum și rutare multimodală (mers pe jos + transport public).
...
2. Configurează fișierul `.env` pe baza `.env.example`
3. Rulează aplicația folosind un server local (ex: XAMPP)
4. Accesează aplicația în browser
5. Rulează scriptul Python pentru regenerarea grafului:
...
* Rutarea pe graf propriu este disponibilă doar pentru municipiul Brașov
  (din motive de dimensiune și performanță)
...
Acest proiect este realizat ca lucrare de licență și reprezintă un prototip funcțional.
Accentul a fost pus pe algoritmică, integrarea datelor reale și demonstrarea conceptului.